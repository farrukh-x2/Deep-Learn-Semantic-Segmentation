...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    up_sampled_1 = tf.layers.conv2d_transpose(conv1x1, num_classes,4, 2, padding='same',
                              kernel_regularizer=kernel_regularizer,kernel_initializer = kernel_initializer)
    #print(conv1x1.get_shape,up_sampled_1.get_shape)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        for images, labels in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict ={input_image:images, correct_label:labels, learning_rate:l_r, keep_prob:kp_pr})
             
            print("cost =", "{:.9f}".format(loss))
...
def run():
    ...
    batch_size = 8
    correct_label = tf.placeholder(tf.int64, (None,None,None, 2))
    data_dir = './data'
    runs_dir = './runs'
    ...
    with tf.Session() as sess:
        ...
        lambda_for_func = lambda imgg: helper.single_img_inference(imgg, image_shape, sess, image_input, logits, keep_prob)
